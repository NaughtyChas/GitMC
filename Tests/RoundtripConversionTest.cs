...
public class RoundtripConversionTest
{
    ...
    public async Task<bool> TestRoundtripConversion(string originalMcaPath)
    {
        try
        {
            ...
            await GenerateTestReport(originalMcaPath, snbtPath, reconstructedMcaPath, verificationResult,
                comparisonResult);
    ...
    private async Task<McaVerificationResult> VerifyMcaFile(string mcaPath)
    {
        try
        {
            ...
            {
                IsValid = true, ChunkCount = chunks.Count, FileSize = fileSize, ErrorMessage = null
            };
        }
        catch (Exception ex)
        {
            return new McaVerificationResult
            {
                IsValid = false, ChunkCount = 0, FileSize = 0, ErrorMessage = ex.Message
            };
        }
    }
        ...
        try
        {
            ...
            if (originalChunks.Count != reconstructedChunks.Count)
                return new ChunkComparisonResult
                {
                    Success = false,
                    Message =
                        $"Chunk count mismatch: original {originalChunks.Count}, reconstructed {reconstructedChunks.Count}"
                };

            // Basic comparison - could be extended for detailed NBT comparison
            return new ChunkComparisonResult
            {
                Success = true, Message = $"Chunk count matches: {originalChunks.Count} chunks"
            };
        }
        catch (Exception ex)
        {
            return new ChunkComparisonResult { Success = false, Message = $"Comparison failed: {ex.Message}" };
        }
    }
