...
public static class Snbt
{
    ...
    {
        return (sbyte)tag.Value + OptionalSuffix(options, ByteSuffix);
    }

    public static string ToSnbt(this NbtShort tag, SnbtOptions options)
    {
        return tag.Value + OptionalSuffix(options, ShortSuffix);
    }

    public static string ToSnbt(this NbtInt tag, SnbtOptions options)
    {
        return tag.Value.ToString();
    }

    public static string ToSnbt(this NbtLong tag, SnbtOptions options)
    {
        return tag.Value + OptionalSuffix(options, LongSuffix);
    }
    ...
    {
        return QuoteIfRequested(tag.Value, options.ShouldQuoteStrings, options.StringQuoteMode,
            options.NewlineHandling);
    }
    ...
    {
        if (options.Minified)
            return ListToString("", x => x.ToSnbt(options), tag, options);
    ...
    public static string ToSnbt(this NbtCompound tag, SnbtOptions options)
    {
        ...
        if (options.Minified)
        {
            ...
            foreach (var childTag in tag)
            {
                ...
                sb.Append(childTag.ToSnbt(options, true));
                first = false;
            }
    ...
    private static string ListToString<T>(string listPrefix, Func<T, string> function, IEnumerable<T> values,
        SnbtOptions options)
    {
    ...
    private static string QuoteIfRequested(string str, Predicate<string> shouldQuote, SnbtOptions.QuoteMode mode,
        SnbtOptions.INewlineHandler newlines)
    {
        if (shouldQuote(str))
    ...
    private static void AddSnbt(NbtTag tag, SnbtOptions options, StringBuilder sb, string indentString, int indentLevel,
        bool includeName)
    {
        if (tag is NbtCompound compound)
        {
    ...
    private static void AddSnbtCompound(NbtCompound tag, SnbtOptions options, StringBuilder sb, string indentString,
        int indentLevel, bool includeName)
    {
        AddIndents(sb, indentString, indentLevel);
        if (includeName)
    ...
    private static void AddSnbtList(NbtList tag, SnbtOptions options, StringBuilder sb, string indentString,
        int indentLevel, bool includeName)
    {
        AddIndents(sb, indentString, indentLevel);
        if (includeName)
