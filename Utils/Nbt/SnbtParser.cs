...
internal class SnbtParser
{
    ...
    private static readonly Regex DoublePatternNosuffix =
        new("^([-+]?(?:[0-9]+[.]|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?)$", RegexOptions.IgnoreCase);

    private static readonly Regex DoublePattern =
        new("^([-+]?(?:[0-9]+[.]?|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?d)$", RegexOptions.IgnoreCase);

    private static readonly Regex FloatPattern =
        new("^([-+]?(?:[0-9]+[.]?|[0-9]*[.][0-9]+)(?:e[-+]?[0-9]+)?f)$", RegexOptions.IgnoreCase);
    ...
    private NbtTag ReadArray()
    {
        ...
        throw new FormatException(
            $"'{type}' is not a valid array type ({Snbt.ByteArrayPrefix}, {Snbt.LongArrayPrefix}, or {Snbt.IntArrayPrefix})");
    }
...
internal class StringReader
{
    ...
    {
        while (CanRead() && char.IsWhiteSpace(Peek())) Read();
    }
