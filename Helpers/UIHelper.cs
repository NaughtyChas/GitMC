...
internal static class UiHelper
{
    ...
    /// </summary>
    private static void FindElementsOfTypeRecursive<T>(DependencyObject parent, List<T> results)
    ...
    public static void RunOnUIThread(Action? action, DispatcherQueue? dispatcher = null)
    {
        ...
        try
        {
            ...
            if (currentDispatcher?.HasThreadAccess == true)
                action();
            else if (currentDispatcher != null) currentDispatcher.TryEnqueue(() => action());
        }
        catch (Exception)
        {
